...
class FlashviewController extends ComController {
	
	//友情链接
	public function index(){
	
		$User = M('flashview'); // 实例化User对象
	...
	public function edit($id=null){
		...
		$id = intval($id);
		$link = M('flashview')->where('id='.$id)->find();
	...
	public function del(){
		...
		if($ids){
			...
			if(M('flashview')->where($map)->delete()){
	...
	public function update($id=0){
		...
		//轮播图标题
		$data['cn_title'] = I('post.cn_title','','strip_tags');	
		$data['en_title'] = I('post.en_title','','strip_tags');	
