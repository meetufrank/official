//初始化常用城市
var commoncitys=window.__arr__;




//初始化所有国内城市
var citys=window.__arr2__;
// A
...
(function($) {
    ...
    $.suggest = function(input, options) {
        ...
        if($.trim($input.val())=='' || $.trim($input.val())=='中文/拼音') $input.val('中文/拼音').css('color','white');
        ...
        function resetPosition() {// requires jquery.dimension plugin
        ...
        function displayItems(items) {
            ...
            if (items=='') {//热门城市遍历
                for(h in options.hot_list){
                    html+='<li rel="'+options.hot_list[h][0]+'"><a href="'+options.hot_list[h][0]+'"><span>'+options.hot_list[h][2]+'</span>'+options.hot_list[h][1]+'</a></li>';
                }
                ...
                for (var i = 0; i < options.source.length; i++) {//国内城市匹配
                    //var reg = new RegExp('^' + items + '.*$', 'ig');
					var reg = new RegExp(items, 'ig');
		
                    if (reg.test(options.source[i][0]) || reg.test(options.source[i][1]) || reg.test(options.source[i][2]) || reg.test(options.source[i][3])) {
                        html += '<li rel="' + options.source[i][0] + '"><a href="' + options.source[i][0] + '"><span>' + options.source[i][2] + '</span>' + options.source[i][1] + '</a></li>';
                    }
                }
                if (html == '') {
                    suggest_tip = '<div class="gray ac_result_tip">无：' + items + '</div>';
                }
                else {
